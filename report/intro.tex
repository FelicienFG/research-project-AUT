...
no scalable optimal algorithm exists. Therefore, either heuristics are 
used to partially solve the problem,
or the optimal but not scalable Integer Linear Programming
(ILP) method is used.

Another prevalent task model use to 
model the dependencies between tasks
in an event-chain is the Directed Acyclic Graph (DAG) task model
which is described in the next section (Section \ref{sec:bg}).
