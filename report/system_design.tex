...
Every $W$, $a$, and $b$ represent a different matrix of trainable parameters,
...
It is this solver that will be slightly modified and used for computing the labels.
The main modification is to replace the objective function,
which initially was the sum of dependency delays (see the paper \cite{yip2023letsynchronise} for more details),
 by the end execution time
of the sink node (i.e., the makespan) of the graph.
This effectively makes the program minimize the makespan for an input graph
...
To convert the input DAG task to an event-chain of LET task,
each node of the graph is considered as a LET task with the 
LET interval being the wcet of the node and every node of a DAG task
have the same period which is two times the total workload of the DAG.
